...
AI 角色扮演的开放框架，骨架是三套自研机制——**异步时序调度系统、TAG 过滤与声明式占位符组装、自定义变量与数值规则（WIP）**——真相存于结构化事件日志而非聊天文本。LLM 调用均为无状态 activation（GM/角色/正文），不构成自主 Agent；Agent 版本是基于这些机制的远期产品层（见 ROADMAP P3），不是框架本体。
...
```bash
npm run dev        # CLI 游玩（tsx src/cli.ts，已冻结：WebUI 是唯一交互界面，CLI 不再是设计对象）
...
- `docs/` — 全部项目文档（见「文档体系」）
- `data/assets/{setId}/` — 世界包：`lores.json`（世界书，条目 = {id, content, enabled?} 全末端外壳）/ `incident.json`（突发公式配置，参数唯一出处）/ `tags.json`（TAG 注册表）/ `vars-template.json`（变量模板）/ `vars-tags.json`（TAG 附加文件；三者装配时读取、缺文件拒装，新会话拷入存档 `sys.json`）/ `player.json` / `characters/*.json` / `prompts/*.prompt.json`（四份：character/gm/prose + gm-incident 突发变体；世界设定/基调卡静态文本内嵌在模板静态文案里）+ `prompts/placeholders.json`（声明式占位符目录，缺文件拒装；示例包 `baitan`，新会话可选）
- `data/users/{username}/` — 用户资源：`secrets.json` / `api-presets/` / `settings.json` / `save/{runId}/` 存档（Generation 布局：`CURRENT` + `generations/{revision}/` 七真相文件，旁路产物留 run 根）
...
- **提示词模板档内化，但代码不热重载**——模板编辑（有活跃会话时写档内副本）下一轮激活生效；改代码后必须重启服务再新建会话验证（存档版本不符会拒绝加载，这是防"旧代码+新模板"混合态的设计，不是故障）。
...
- 读档内副本原则：新会话把世界包 lores.json 拷入存档 `save/{runId}/` Generation 内的 lores.json，运行期增删改只动副本，不污染 `data/assets/` 原始世界包。
