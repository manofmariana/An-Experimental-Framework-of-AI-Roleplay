...
- `location` 是结构化变量 `{name, level}`：name 是 GM 裁决时赋予的自由文本（以世界包为参考、提示词管理），同地判定按 name；level 供突发鉴定使用（鉴定本身未接线，见 ROADMAP）。
...
- **模板** = `data/assets/{setId}/prompts/{agent}.prompt.json`：每包完整三份副本（character/gm/prose，无兜底）；有序模块数组，模块 = `{key, role: system|user|assistant, content}`，content 内 `{{placeholder}}` 占位；Web 可编辑（「提示词」页签，`?set=` 定位包），**每轮激活前热加载**——保存后下一轮即生效。
