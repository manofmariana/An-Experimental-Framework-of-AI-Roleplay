...
- 工作集落盘于 `sys.json` 的 `pipeline.working_set`，随 GM 裁决清算（通知条目同灭）；GM 恒见（权重 6 恒过）。条目并集 = 言行条目 | 系统通知条目：
