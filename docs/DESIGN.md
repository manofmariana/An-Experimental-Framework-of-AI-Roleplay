...
- `sys.json`（第五根）：`{schema_version, tagRegistry, varsTemplate, varsTags, cycles_since_gm, gm_trigger, gm_trigger_batch, pipeline:{seq, working_set, current}}`——档内结构三件套 + 程序计数键（经 `sys.*` VarChange 通道落账）+ 步边界 pipeline；schema_version 是全档唯一盖章点。pipeline 永不进入 agent 的 world 快照。
