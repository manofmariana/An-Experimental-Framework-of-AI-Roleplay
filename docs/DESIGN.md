...
> 定位：以**异步时序调度、TAG 过滤与声明式占位符组装、自定义变量与数值规则**三套机制为骨架的 AI 角色扮演框架——取代 SillyTavern 式单上下文结构，实现更好的上下文管理、缓存命中率、多角色认知隔离与即时功能拓展。LLM 调用均为无状态 activation，不构成自主 Agent；Agent 版本是基于这些机制的远期产品层（见 ROADMAP P3），不是框架本体。

---

## 1. 设计哲学

### 1.1 SillyTavern 式结构的权衡

ST 式结构 = **单上下文、单人格、全知视角的文本拼接**：

1. 单体 prompt 组装：角色卡 + 世界书 + 全部历史每轮拼成一个大字符串，一个模型同时扮演所有角色。
2. 前缀缓存难命中：世界书按深度插入、历史尾部增长，prompt 前缀每轮都在变化。
3. 无认知隔离：模型天然全知，秘密/谎言/悬疑/戏剧反讽只能靠提示词约束维持。
4. 聊天文本即状态：改一条历史全靠手动编辑文本，无结构化一致性校验。
5. 扩展多以前端补丁形式存在：前后端耦合较紧，新能力开发成本高。

### 1.2 本架构的四条哲学

1. **作者退后，角色自治**：角色是独立的人，不知道自己在作品里。主要角色的台词、决策、记忆由角色自己的 activation 全权负责，任何其他 activation（包括 GM）不得替角色行动、感受、思考。
2. **真相层与视图分离**：结构化事件日志 + 变量库 + 知识账本是唯一真相；正文、角色感知、前端画面都只是真相的渲染视图。视图永远不许反向污染真相。
3. **需要判断的才调用 LLM，纯工序做成确定性服务**：变量落库、感知投递、计时器、骰子——零 LLM。LLM 调用只花在判断和创作上。
...
```
              玩家输入（主控意图）
                      ...
                      ▼
        ┌──────────────────────────────┐
        │ 异步时序调度（连续时间轴 DES）  │  机械层（零 LLM）：
        │ 组/先攻/行动周期全派生         │  到期弹出、组划分、计时器、
        └──────────────┬───────────────┘  感知过滤、变量落库、标记执行
                       ▼
        ...
        （私域上下文，唤醒制，           （唯一全知者，重裁决/      （纯渲染器，无工具，
         自主决策台词/行动/内心）         世界反应/@ID 转写）        无权改写角色台词）
                       │
                       ▼
        瘦前端（纯渲染器；CLI + WebUI 双实现）
```

注：角色/GM/正文 activation 是无状态调用——每次激活 = 从数据层组装提示词的全新请求，无对话历史、无跨调用缓存、无工具调用能力，不构成自主 Agent。多角色"人格"是调度 + 过滤 + 组装三套机制叠加出的效果；Agent 版本（工具调用能力）是基于这些机制的远期产品层（ROADMAP P3），不是框架本体。
...
- **突发变体**：同一身份的第二种提示词组（矩阵 `gm.incident` 功能组）+ slim 契约（事件文本 + 可选 deltas，独立轻校验）——突发事件机制见 §4.8。

### 3.3 正文 activation（纯渲染器）

- **无工具、无检索权**。世界知识 = 静态世界基调卡 + 世界设定 + 触发 lore（本轮参与者有效 TAG 集并集按 TAG 求值激活的条目，去重按 ID 排序）。
- 注入顺序：协议（含基调卡）→ 世界设定 → 近期事件（演员表渲染）→ 演员表（CID 与名字）→ 触发 lore → 上一轮正文（行文衔接）→ 本轮各角色台词+内心（inner 含意图，作情绪参考；标记不注入）→ 本轮 GM 事件包。正文不注入 GM 公共长期记忆；台词原文所有权在角色 activation，**无权改写**。
- 渲染范围 = GM 包全部事件（一次 GM 包至多一次正文调用，narrativity 包级，skip 则无正文；多组交替 = 蒙太奇叙事，不做玩家视角过滤）。
- **指称占位符**：正文存带 `[[称呼|@CID]]` 的原文（直接引语豁免），渲染看读者——玩家显示渲称呼、角色注入按 relations 渲染、GM 注入渲"称呼（@CID）"、正文 activation 自身的上一轮正文保持原文（格式学习通道）。与事件的 @CID 存储 + 身份替换同一"存储用 CID、渲染看读者"体系（adr/0003）。
- **防出戏**：① 世界基调卡（元设定，静态进前缀，挡 gross 级出戏）；② 触发 lore 条目（挡场景级出戏；秘密条目访问控制 = 标签，不持有对应标签即结构上不进正文）。
- **防添油加醋**：① 硬约束块（必须且只能覆盖本轮 GM 事件）；② 标签隔离；③ 信道分离——台词与关键动作所有权在角色 activation，正文只写结缔组织（最强的一道，结构性保证）。
...
- **变量树存储**：全部变量存为变量树（容器/结构化数组/末端三类节点，末端 = `{value, tags, formula?}` 外壳；树内身份由变量模板声明）。`world.json` = 纯变量树（含系统声明分支 time：时间锚五末端 + periods 时段表）；程序内容独立为第五根 `sys.json`（档内 TAG 注册表/变量模板/TAG 附加文件三副本 + 程序计数键 cycles_since_gm/gm_trigger/gm_trigger_batch + pipeline）；角色 `vars` 同理为变量树；角色顶层字段（name 等与调度字段 acted/group/channel/timer/isPlayer/appearance）物理布局不变（调度器继续消费类型化字段、白名单专用通道），经**系统声明分支投影**呈现为同一棵树的标准末端——系统声明子树为代码持有常量（不进世界包模板文件），模板解析时并入 character 根（与世界作者声明同名 = 拒装；world 根同制并入 time 系统分支——时间锚 + 时段表，实例直接在树内、调度经 setClock 专用通道推进），投影值从类型化字段读出（timer/channel null 原样、initiative null = 容器无实例、relations 数组按下标投影）；所有末端（含系统字段）可挂内容侧 tags，系统末端的外壳 tags 存 CharacterState 的 `systemTags` 侧车（只经直编修改，装配/直编时校验 level 1-7 + 注册名集合；GM deltas 拒写系统分支）。tags 只存在于末端外壳——数组节点与数组元素对象自身没有挂载位。
...
- **变量模板与 TAG 附加**：变量模板（世界包 `vars-template.json`，档内副本 = sys.json 的 varsTemplate）= `{world, character, types}` 三棵声明树——容器（`{children}`；子键 = 人类可读名称，禁保留子键 value/tags/formula 与下标括号）/ 结构化数组（`{array: 元素声明}`，元素 = `{type}` 引用 types 结构别名或 `{children}` 内联对象结构；元素根不得又是数组；实例 = 元素对象数组，实例键废弃）/ 末端（`{valueType, formula?}` 或 valueType 字符串简写；valueType = number/string/boolean/string_list/tag_list）；types = 纯结构别名注册表（每类型只能 `{children}`，引用必须可解析且无环，直接与间接递归都拒）。结构编辑与实例写值解耦（只加声明不写值合法；无声明有实例 = 校验拒绝）；character 声明树全体角色共享，根保留名 `attachtags`（固有 TAG 末端 = string_list 纯名集合，GM 挂 TAG 的直写点）+ `tags`（union 从动池 = string_list：terms = attach 子树项 + cid/location/channel 系统字段常驻项）。TAG 附加文件（世界包 `vars-tags.json`，档内副本 = sys.json 的 varsTags）与模板同构：节点 = `{tags?, children?}` 或 `{tags?, array}`（数组整型挂载，array = 元素类型名、内联元素为 `"*"`）；节点级条目扇出到其下全部末端（根节点自身也可挂条目 = 级联到该根全部末端；数组层路径以 `[*]` 占位）、末端级条目只挂该末端，按模板末端位置解析映射实例路径，cid 类条目按实例属主分发（character 根挂 {category:"cid"} = 每角色全部末端挂自身 CID）；附加 TAG 读取期与实例 tags 合并、不物化进实例值（消费在提示词组装层）；状态编辑器把合并结果以只读「附加」chips 并入各末端 chips 区显示（前端镜像 resolveAttachTags：扇出/单挂/`[*]` 按下标通配匹配/cid 按当前 scope 角色分发，world 域无属主遇 cid 条目跳过；只作显示，绝不写进实例值/保存载荷/侧车）。
- **从动级联**：任一变量根（world 域或某角色）写落后，按该根依赖图拓扑序整根全量重算全部从动末端（expr 数值公式 + `union` 内置算子；从动集合小，不做细粒度失效分析），值变则写回并把该末端的 VarChange 追加到同段 changes（回溯/重放天然覆盖）。union 公式 = terms 组合（按序并集、按名去重先取者胜）：`{attach: paths[]}` 项 = 自身 attachtags ∪ 各子树路径下全部 attachtags 末端值（paths 须解析到容器/数组声明，建依赖图边——粗粒度：子树任意写入触发重算）；`{sys: "cid"|"location"|"channel"}` 项 = 读属主角色系统字段（cid → [cid]、location → [当前地点名]、channel → 无频道时 [] 否则 [频道号字符串]），只允许出现在 character 根且 terms 内不重复，**不产生图边**——系统字段变化经 `CharactersStore.setVars` 白名单通道在 location/channel 变更后重算该角色池（值变才追加池的 VarChange），存档安全网（validateSaveSet）每次提交按档内模板重算全部角色池比对兜底（adr/0016）。计划 = 模板声明 formula ∪ 实例携带 formula（实例覆盖同路径模板声明）；结构化数组在计划中按 `*` 通配段展开、重算时对实例根做元素下标枚举（characters 根等 cid 键控记录仍枚举键——两种通配语义按实例形状分派）；数组元素结构内的 formula 以元素结构根为基准声明，并入计划时按挂载路径补齐前缀；expr 依赖末端无实例（取不到值）时该末端跳过重算（保持现值）。依赖成环 = 拒装/拒写（装配/续档/直编共用 sys 根严格解析出口 parseSys，成环闸随解析做一次）。直编替换后两域全量级联——被直编的从动值回归计算值，级联结果并入同一次 commit。程序消费角色 TAG 集只读 `vars.tags` 池（string[] 纯名集合；union 从动末端，走同一级联，无特判路径——自身 cid/当前地点名/当前频道号经 sys 项常驻池内，过滤层不再读时拼接）。
...
- **GM 挂 TAG = 普通变量写**：deltas 直写 `characters.{cid}.vars.attachtags`（string_list 纯名数组全量替换；注册表名称校验，非法拒绝该条）；tags 池由 union 级联自动维护（§3.4）。
- **四根过滤统一收进引擎**：events/lores/characters/world 四大内容根同构——末端一律 {value, tags, formula?} 外壳，投影层全量供给（仅 prose 读者两处取数范围截取：事件滑窗与 lores 参与者触发集），引擎路由 `{events[*].content}` 等全路由链路径逐末端过求值器（在场性 fappear 虚拟挂载同前）；求值器、注册表、名称校验同一口径。管线接线见 §7.1。
...
- **连续时间**（硬性要求，不做时间量子化）。世界时间落盘唯一表示 = world 变量树的 time 系统分支（`world.time` = 时间锚 {y,m,d,h,min} 五末端 + periods 时段表结构化数组）；逻辑 clock 以 y=365d、m=30d 从锚派生绝对分钟；最小单元 1 分钟。初始时间 = 新档 world.time 锚（代码缺省原点开局，世界作者经状态直编调整，档内随 world.json）。自然语言时间头（"N年M月D日·时段"）经 world.time 的 periods 时段表机械渲染，LLM 不自由生成。
...
3. **感知过滤**：事件可见性 = 引擎对 `{events[*].content}` 逐末端按读者有效 TAG 集求值（落盘池 ∪ 自身 cid，adr/0002；投影全量供给）；绝对时间只在真相层，提示词中的时间/地点经占位符注入。
...
- **突发 GM**：身份 = GM（同一预设、同一全知视野），专用提示词组 = 矩阵 `gm.incident` 功能组（"同一身份 × 不同功能 = 不同提示词组"首个实例）；**slim 契约** = 事件文本 + 可选 deltas，独立轻校验，不复用 durations 覆盖校验。
...
- **事件元素 = {id, t, seq, kind, location?, content} 全字段末端外壳**（{value, tags, formula?}，与变量树末端同构；时间和地点同样是可挂 tags 的调取对象——实际往哪些字段挂 tags 归 GM 输出契约）。可见性 TAG 挂载在 content 末端：常规 = cid 类一级（谁实际感知挂谁），空数组程序补全本轮全部行动者；感知过滤 = 引擎按读者有效 TAG 集逐末端求值，无地点成分。私密事件 = 只挂自身 cid。
- **事件头**：t 存结构化 clock 值、location 存文本（各为独立末端外壳）；自然语言时间头在注入/显示时经 world.time 的 periods 机械渲染。跨场景对话事件 location 置空（感知不靠它）。
- 真相是第三人称的，**呈现是第一人称的**：编译给角色的内容经身份替换转写为角色本人的认知。

### 5.2 变量库

- `world.json`：`{world: 纯变量树}`——含 time 系统分支实例（时间锚五末端 + periods 时段表）与作者世界变量树；clock 由 world.time 锚派生不落盘；GM deltas 拒写 world.time（调度经 setClock 专用通道）。
- `sys.json`（第五根）：`{schema_version, tagRegistry, varsTemplate, varsTags, cycles_since_gm, gm_trigger, gm_trigger_batch, pipeline:{seq, working_set, current}}`——档内结构三件套 + 程序计数键（经 `sys.*` VarChange 通道落账）+ 步边界 pipeline；schema_version 是全档唯一盖章点。pipeline 永不进入 activation 的 world 快照。
- `characters.json`：`{characters:{cid: CharacterState}}` 单文件，C0 与 NPC 同构——cid/name/gender/age/personality/reaction/location/timer/group/initiative/level/omniscience/isPlayer/appearance/relations/long_term_memory/systemTags/vars（**cid = 自身 CID 字符串，建角时物化（= manifest.id），系统只读**（deltas/varWrite 拒写，与调度字段同口径），投影视图呈 characters 根 `cid` 末端）；appearance = 在场位（程序维护，vars 源在场性过滤的输入）；relations = `{cid, name?, impression?}[]` 数组；vars = 变量树：末端 `{value, tags, formula?}` 外壳；角色根保留名 attachtags = 固有 TAG 末端、tags = union 从动池（terms 含 cid/location/channel 系统字段常驻项））。角色只注入自己的完整快照，GM 注入全部角色快照。

### 5.3 存档（Generation 布局，统一 schema_version）

- 路径 `data/users/{username}/save/{runId}/`：`CURRENT`（文本 = 6 位零填充 revision）+ `generations/{revision}/` 七真相文件 = world/characters/events/lores/archive/sys/prompts（四大内容根 + sys 第五根 + 独立的 archive/prompts）。**schema_version 单点化**：只在 sys.json 盖章，版本闸只读 sys 一处（缺 sys.json 或字面量不符 = 拒绝加载且不迁移），其余文件 codec 不携版本字面量。`prompts.json` = 档内提示词模板副本：新会话把世界包 `prompts/` 矩阵全量模板（`{object}.{function}.prompt.json` = character.decision / gm.adjudication / gm.incident / prose.render）校验后拷入存档（缺文件/校验失败即拒装），运行期编辑只动副本。
- **步边界整代提交**：各 Store 是纯内存容器，唯一写盘出口 = `generationRepository.ts`，唯一提交入口 = `commitExecutor.ts`（CommitPlan = transactionId/baseRevision/reason/changes，plan 不落盘）。原子提交：临时目录 → 重读校验 → rename → CURRENT 切换；保留 current+previous，加载遇损坏回退上一代（灾备）。
...
- **状态直编**（Web 状态栏）：world/characters/events 三域校验后整体替换（任一失败整体还原），LLM 在途拒绝；变量域经 varDiff 净额并入当前步 changes.effects（archive 记跨步净变更，手动编辑不是独立变更记录），事件域按 seq 截断口径；提交前跑一次组派生（rederiveGroups，幂等保稳，组未变则零变更）——直编对齐 timer/location 立即反映到编组；派生现算无缓存，编辑即时生效。直编 modal =「变量 / 事件」两标签页（一次只显示一个）：变量页 = 树状**状态编辑器**（世界/角色切换；time 系统分支不显示（程序维护）；系统只读 = {acted, group, channel, timer, isPlayer, appearance}，其余角色顶层字段按内置声明表可编辑——omniscience 前端钳制 0-6、initiative 为 null 时两值齐全整体写回、relations 条目增删（按 cid 追加/按下标摘除）、long_term_memory 行编辑；状态操作 = 末端写值 / 外壳 tags 与 tag_list chip 编辑（条目名称取自档内注册表）/ 结构化数组元素增删（按元素结构物化空白、只动实例不动模板）；从动末端值只读、formula 只读标注；保存载荷 = 纯变量树工作副本（sys 结构编辑不经本通道）；直编后两域全量级联，从动值回归计算值），事件页 = raw JSON。modal UX：整树重渲保持滚动容器 scrollTop 不跳顶；保存成功不关窗（行内「已保存」短提示 + baseRevision 用保存后新 revision 推进，失败 400/409 行为不变）；取消/点遮罩在有未保存修改时先 confirm 确认。**变量结构编辑归世界页，双模式**：打开时探 GET /api/session/state/sys——有活跃会话 = **档内模式**（数据源 = 会话 sys 根的 varsTemplate/varsTags/tagRegistry + baseRevision；保存 = PUT /api/session/state 带 `sys: {varsTemplate, varsTags}` + baseRevision 乐观闸，sys 原样并入直编载荷走同一通道——parseSys 严格解析 + normalize + 从动级联沿用，结构不合法 400 零落盘，409 提示重取；保存后立即生效，TAG 附加注册表数据源 = 档内 sys 根 tagRegistry），无活跃会话（404 NO_ACTIVE_SESSION）= **包基线模式**（vars-template.json / vars-tags.json，PUT 经 parseVarsTemplate / parseVarsTags 对拍校验、失败 400 零落盘、markStale 新会话生效；缺文件 GET 回缺省空结构、PUT 创建；注册表 = 包 tags.json）；模式指示行常显，编辑器本体两模式复用。编辑器本体：变量模板子区 = 声明树结构编辑（扁平末端五 valueType/结构体/结构化数组（引用类型或内联元素结构）新增、声明删除、类型新建/逐字段定义/删除（被数组元素 {type} 引用类型拒删）、末端 formula 声明编辑/清空（expr + binds / union terms = attach 子树路径项 + sys 系统字段项三勾选），character 域 = 全体角色共享模板、根保留名 attachtags/tags 保护、**character 根显示并入系统声明分支**（系统节点带徽记、全部结构操作禁用——只是显示注入，绝不写回保存载荷；与系统键同名的新增拒绝；formula 校验按并入后根解析，作者公式可绑系统 number 末端）），TAG 附加子区 = 声明树节点挂附加条目（{name/category, level}，名称下拉自当前模式注册表；character 根投影同样并入系统声明分支，系统节点可挂附加条目——与服务端 parseVarsTags 按并入后根校验口径一致；结构化数组整型 {tags, array} 挂载，扇出到元素结构全部末端）。
...
- 工作集落盘于 `sys.json` 的 `pipeline.working_set`，随 GM 裁决清算（通知条目同灭；指令条目豁免，见下）；GM 恒见（权重 6 恒过）。条目并集 = 言行条目 | 系统通知条目 | 指令条目：
  ...
  - **指令条目** = `{id, author, directive:{mode:"god"|"writing", text}, tags}`：**输入模式**（主控/上帝/写作）中上帝/写作两条指令通道的内容单元——主控 = 玩家直接撰写决策包的现状通道（player_input → 言行条目），不动；上帝指令 = 玩家对世界/剧情的元层指令，写作指令 = 对正文文风/写法的元层指令。提交通道 = WS `directive {mode, text}` 命令 → Coordinator 裸队列（llmBusy 即拒；不自动建会话）→ author = 主控角色 cid → 并入工作集（id 同 mode 固定复用 `directive:{mode}`，后来者居上），一次提交、不产生 archive 步。tags 创建时挂 `{强制全知, 7 级}`（结构就位，实际供给不过 TAG 求值）。**供给 = 读者轴**（组装源 `god_directive` 仅 GM 读者 / `writing_directive` 仅正文读者，逐条供给、owner = author；角色读者恒空——读者轴供给本身就是第四面墙）；文本原样透传（出厂基线条目段 identity:false）；渲染层 renderScene/renderSpeech 一律跳过（不泄进场景/台词文本），注入给谁 = 模板把占位符放在哪（出厂基线：gm.adjudication 尾部上帝指令模块——本轮有效、尽力服从、与裁决协议冲突时协议优先；prose.render 尾部写作指令模块——与模板既有要求冲突时模板优先；gm.incident 不注入——突发在轮收尾后触发，指令已清算）。**生命周期 = 当轮一次性**：豁免 GM 清算（narrativity ≠ skip 时保留——正文读者的场景文本不走工作集，写作指令须活到 prose 步；narrativity = skip 无正文步，随工作集全清），正文步提交（含被停止后的编辑补全）清除残留；回溯重建无凭据（不产生 archive 步），越过提交点即消失（取舍见 adr/0015）。
...
- **模板** = 档内第七真相文件 `prompts.json` 的 `templates` 键（出厂基线 = 世界包 `data/assets/{setId}/prompts/{object}.{function}.prompt.json`，新会话拷入存档，此后只动档内副本）：**对象×功能矩阵全量**（对象轴 = activation 代码枚举 {character, gm, prose}；功能轴 = 每对象封闭枚举 character·decision / gm·adjudication·incident / prose·render；映射表 = `truth/promptsStore.ts` 的 PROMPT_MATRIX 单一代码常量，无兜底）；有序模块数组，模块 = `{key, role: system|user|assistant, content}`，content 内 `{{placeholder}}` 占位；Web 可编辑（「提示词」页签，对象→功能组两级页签）：有活跃会话时读写档内副本——保存后下一轮激活即生效，无会话时读写包基线（`?set=` 定位包）——新会话生效。
- **占位符全声明式、定义读者无关**（`prompts.json` 的 `placeholders` 键；出厂基线 = 世界包 `prompts/placeholders.json`，缺文件/校验失败拒装）：条目 = `{description, source?, segments[]}`——**source 二分类**：程序组装类（source 在场，封闭枚举：working_set/prose_window/last_prose/clock/cast/contacts/departure_notices/incoming_contact/timers/fortune/gm_event/incident_target/world_snapshot/snapshot/group_members/long_term_memory/god_directive/writing_directive；每个组装源暴露 content/owner 两个固定可调用末端，命名路径 `{<source>.content}` / `{<source>.owner}`）｜ 本地落盘四根（source 缺省，路径首段即根——events/lores/characters/world；模板直接写全路由链路径，如 `{events[*].content}`）。段 = 静态文本（原样输出）/ 条目（`{pass, fail?, order?, identity?, separator?, merge?}`——pass/fail 各为缺省注入模板 + "匹配记号集 → 模板"精确匹配分支，未命中走该侧缺省兜底；fail 缺省 = 空模板；**identity:false = 关闭本段身份替换后处理**（缺省 = 做；服务需要直接输出 @CID/cid 原文的条目））。落盘根遍历结构 = 引擎沿路由链找差异点自动归并（公共前缀共享、`[*]` 差异点产子循环，无独立轴声明）；events/lores 的元素结构 = 系统固定结构（代码内登记供机检）。遍历序前置（默认，条目轴独立滚完）/ 置后（同首位轴的置后条目融合为逐实例组；同占位符内置后条目首位轴必须一致）。编辑期机检（装配/续档与包基线校验同一函数）：落盘路径必须解析到末端（tag_list 原子即止；非法首段/穿越末端拒）、组装源只许命名路径、多路径路由链最前差异点兼容、置后同轴、分支记号 ∈ TAG 注册表条目名。未知占位符在模板加载/保存时报错（合法键集 = 目录键集）。Web 可编辑（「提示词」页「占位符」页签，双模式与生效口径同模板编辑）：列表态 = nameplate 陈列（名 + description 摘要；点牌/加号牌进编辑子页面），子页面 = 名/description/段列表单（条目段带「关闭身份过滤」勾选）+ 模板字段「引用」钮级联寻路（**顶层两大类 = 路径 / 程序**：路径 = 落盘四根沿结构逐级菜单——events/lores 为前端镜像固定结构、characters/world 按当前模式 varsTemplate；程序 = 组装源列表 → content/owner 两末端），选定末端在光标处插入原子引用 chip（整体复制/剪切/删除、内部不可编辑），存储仍序列化为 `{路径}` 纯文本；**source 不设手选控件——保存时由全部路径调用推导**（组装类路径全同一 source 且不混落盘根，违反 = 本地报错不提交；服务端机检同口径）——整份提交（PUT /api/prompts/placeholders）过 parse + 编辑期机检（机检上下文按模式供给：档内 = 档内 sys 根模板与注册表，包基线 = 该包变量体系文件），失败 400 零落盘。
- **管线次序 = 内容源投影 → TAG 过滤 → 声明式渲染 → 身份替换后处理**：投影层（`src/application/activationContexts.ts`，RenderHost 实现）按读者限定取数范围（落盘四根全量供给——仅 prose 读者两处截取：事件滑窗（取数范围不是过滤）与 lores 参与者触发集（保正文 lore 触发制）；working_set 经同值批隔离 + 逐条目 TAG 过滤（§6.1）——读者有效 TAG 集 = tags 池纯名集 ∪ 程序派生（自身 cid/当前地点名/当前频道编号/工具 AV 临时挂载）+ 全知权重；GM/正文 = 权重 6 恒过；cast 现建、lore 逐调用读档内副本，全部从最新真相逐调用现算）→ 引擎逐末端过 TAG 求值器（四根同一路径）——**在场性 = appearance 系统布尔 + 虚拟挂载**：对 `appearance=false` 角色的全部末端虚拟挂载 `{fappear, 6 级}`（不落盘、等级代码焊死；权重 6 全知恒见，权重 0-5 读者须持 fappear 纯名方可见后台；自豁免：属主 = 读者的子树不挂载——邀请应答时受邀者仍在后台，是后台读者的唯一场景）；程序维护时点 = 组弹出前台置 true（轮首 setup 段）、结算进后台置 false（GM 裁决）、入组置 true（远程按组籍/召回）、离组置 false（leave/频道清理），全部走 writeRaw 随 changes 落账；characters 根每角色 = 系统分支投影（cid/name/location 等类型化字段呈标准末端，tags 取自 systemTags 侧车）+ vars 树，**world/characters 两根供给前做 vars-tags 附加的读取期合并**（resolveAttachTags 按属主分发 + 并入末端 tags，不物化进实例值）；放行框全部路径放行 → 放行侧，任一不放行 → 不放行侧，不放行侧路径照常解析、未放行的给空）→ 引擎（`src/compile/render.ts`）渲染（懒求值：未引用占位符不取数；渲染后空模块整条丢弃；空渲染实例/条目在拼接处整条丢弃）→ 身份替换统一后处理（组装源 identity 条目与 events 根 string 末端过 renderIdentity，**条目段 identity:false = 关闭本段身份过滤**：角色 = renderForReader/renderRefsForReader；GM = 事件保持 @ID 原文（cid 模式不动）、指称渲染"称呼（@CID）"；正文 = renderForGm 演员表、指称保持原文）。
...
### 7.2 各 Activation 缓存画像

| Activation | 稳定部分 | 每轮变动 | 主要开销 | 缓存要点 |
|---|---|---|---|---|
...
- **触发制**：GM 不为正文激活/禁知条目；正文 lore = 本轮参与者有效 TAG 集（各角色 tags 池 ∪ 自身 cid）并集按 TAG 求值激活的条目（投影供给侧截取，去重、按 ID 排序）；秘密条目访问控制 = 标签（挂在 content 末端，如 `灯塔：秘密` 只随持该标签的角色进正文）；无挂载条目 = 广播恒过。角色/GM 的 lore = 全量供给 + 引擎按读者有效 TAG 集逐末端过滤（GM 权重 6 恒见全量）。条目 = {id, content, enabled?} 全末端外壳；enabled = 管理界面元数据开关，不参与过滤/激活逻辑。
- **档内副本**：新会话把世界包 lores.json 拷入存档 Generation 内 `lores.json`，运行期增删改只动副本，不污染 `data/assets/` 原始世界包；changelog 逐条记录严格可逆变更（回溯按 changelog 反向回滚）。
