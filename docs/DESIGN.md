...
- **决策包契约**：`inner` 必填（内心活动与意图）；`action` 与 `dialogue` 至少其一；关系实际变化时追加 `relations`；调度指令追加 `markers`（五标记结构化数组，见 §4.4）。角色不得输出 span——`Span`（**时长**，相对量）仅属于 GM 裁决包的 `durations` 字段。
...
- 职责：重裁决、次要 NPC 代演、世界反应、为每个行动者给出**时长**（`durations` 的 span，相对量；程序锚定当前世界时钟折算为到期时刻）、设置 location、以 `known_by` tags 逐事件标记可见性（唯一通道，adr/0002）、**narrativity**（包级，进入正文的权重）。
...
- **事件写作纪律**（提示词协议层约束，不做程序强校验）：**摘要制**——不完整转述台词，做摘要与意义总结；禁人称代词（演员表角色一律 @CID）；保言行顺序、只记已发生；多条目记录事实而非视角、条目间禁止重合；known_by 谁实际感知标谁；提示词模板示例禁用真实 CID（用 `<CID_甲>` 抽象占位）。
...
- **无工具、无检索权**。世界知识 = 静态世界基调卡 + 世界设定 + 触发 lore（在场角色固定标签激活条目的并集，去重按 ID 排序）。
...
- **变量树存储**：全部变量存为变量树（容器/结构化数组/末端三类节点，末端 = `{value, tags, formula?}` 外壳；树内身份由变量模板声明）。`world.json` 的 world 分支 = `{time, _sys, ...世界变量树}`——`_sys` 是程序分支（模板校验豁免）：档内 TAG 注册表/变量模板/TAG 附加文件三副本 + 程序计数键（cycles_since_gm/gm_trigger 等）；角色 `vars` 同理为变量树；角色顶层字段（name 等与五调度字段 acted/group/channel/timer/isPlayer）物理布局不变（调度器继续消费类型化字段、白名单专用通道），经**系统声明分支投影**呈现为同一棵树的标准末端——系统声明子树为代码持有常量（不进世界包模板文件），模板解析时并入 character 根（与世界作者声明同名 = 拒装），投影值从类型化字段读出（timer/channel null 原样、initiative null = 容器无实例、relations 数组按下标投影）；所有末端（含系统字段）可挂内容侧 tags，系统末端的外壳 tags 存 CharacterState 的 `systemTags` 侧车（只经直编修改，装配/直编时校验 level 1-7 + 注册名集合；GM deltas 拒写系统分支）。tags 只存在于末端外壳——数组节点与数组元素对象自身没有挂载位。
...
- **标记（Marker）**：DecisionPackage 的可选 `markers` 数组，每条 `{type, ...params}`——`gm_request` / `leave`（二者互斥）/ `recall{target}` / `contact{channel, targets[]}` / `confirm`。程序只读结构化字段、不做行内文本解析。**标记不进工作集、不进任何注入**——解析后只有即时程序作用，即抛。
...
- **延迟生效**（防时钟抢跑）：联系标记**触发 GM 立即结算**（GM 与受邀者都看不到邀请标记），邀请在**邀请者所在组下一次回到前台时**才激活异组受邀者（timer 置当前时钟、立即到期弹出应答）；若其他组 timer 更早，他们先行动。
- **拒绝**（输出理由、不立确认标记）→ timer **自动还原**为被邀请前的值，不调用 GM，失去频道变量。**接受**（确认标记 + 首轮回复）→ timer 归当前时钟（立即到期）直到 GM 重设，并入发起者所在组（位置 ≠ 组位置 → 先攻 -1），**首轮回复计入本周期已行动**。
- **频道变量（channel）**：联系标记生效即分配给邀请者与全部对象（同一 id）；拒绝者失去；退组自动清除。职责二合一：**防重入**（频道持有者不进任何联系人列表注入，持有者自身也看不到名单——占位符整体置空）+ **生命周期**（任意两个持有者 location 不同 → 跨场景存在；全同地 → 全清，仍非组位置的持有者离场等结算）。
...
3. **感知过滤**：唤醒时从事件日志取 `time ≤ 唤醒时刻 ∧ tags 含 known_by:自己` 的事件——**known_by 是感知唯一通道**（adr/0002）；绝对时间只在真相层，提示词中的时间/地点经占位符注入。
...
- **事件侧只使用 `known_by` 一类 tag**（角色 ID 即角色标签，与 Lorebook 标签同一套体系）：GM 标记时已推理各角色可视情况；感知过滤 = `known_by` 唯一通道，无地点成分。私密事件 = 可见标签仅含自己。
...
- `characters.json`：`{schema_version, characters:{cid: CharacterState}}` 单文件，C0 与 NPC 同构——name/gender/age/personality/reaction/location/timer/group/initiative/level/omniscience/isPlayer/relations/long_term_memory/vars（relations = `{cid, name?, impression?}[]` 数组；vars = 变量树：末端 `{value, tags, formula?}` 外壳；角色根保留名 attachtags = 固有 TAG 末端、tags = union_attach 从动池）。角色只注入自己的完整快照，GM 注入全部角色快照。

### 5.3 存档（Generation 布局，统一 schema_version）

- 路径 `data/users/{username}/save/{runId}/`：`CURRENT`（文本 = 6 位零填充 revision）+ `generations/{revision}/`（world/characters/events/archive/lore/time 六真相文件，均带同一 `schema_version`，版本不符即拒绝加载且不迁移）。
- **步边界整代提交**：六个 Store 是纯内存容器，唯一写盘出口 = `generationRepository.ts`，唯一提交入口 = `commitExecutor.ts`（CommitPlan = transactionId/baseRevision/reason/changes，plan 不落盘）。原子提交：临时目录 → 重读校验 → rename → CURRENT 切换；保留 current+previous，加载遇损坏回退上一代（灾备）。
...
[近期记忆]   近期具体事件——角色从公共事件库按 known_by 拉取 + 身份替换渲染；GM 拉取全部
...
- 工作集落盘于 `world.json` 的 `pipeline.working_set`，随 GM 裁决清算。
...
- **模板** = `data/assets/{setId}/prompts/{agent}.prompt.json`：每包完整四份副本（character/gm/prose + gm-incident 突发变体，无兜底）；有序模块数组，模块 = `{key, role: system|user|assistant, content}`，content 内 `{{placeholder}}` 占位；Web 可编辑（「提示词」页签，`?set=` 定位包），**每轮激活前热加载**——保存后下一轮即生效。
- **占位符注册表 = 唯一出口**：每 agent 一份 `key → provider(context) → string`；新占位符 = 注册新 provider，不得在组装代码里散落硬编码拼接；未知占位符加载/保存时报错。**provider 纯净化**：只输出数据本身，标题/节名/包装前缀一律写在模板静态文案里。
- **compiler = 纯渲染器**：模板 × 注册表 × 注入上下文 → ChatMessage[]（每模块一条消息，带自己的 role）。
- **注入上下文逐调用现算**（`src/application/activationContexts.ts`）：cast 现建、lore 逐调用渲染档内副本、可见事件/正文滑窗/当前场景/被联系通知全部从最新真相 + 派生投影计算。
- **角色模板不注入演员表**（演员表带真名会让角色不劳而获地知道陌生人名字，污染人际关系库——名字必须靠剧情获得、由角色自己登记 relations）；cast provider 供 GM/正文用。#当前场景一律用 `##@CID` 标注。
...
| 正文 | 基调卡 + 世界 lore + 演员表 | 触发 lore + 近期事件 + 上一轮正文 + 本轮 GM 事件包 | **主输出** | 每轮注册表现取 |
...
- **无状态 activation**：构造只持 ChatPort + 包内 promptsDir（会话级静态）；全量上下文逐调用传入，实例零跨调用缓存。
