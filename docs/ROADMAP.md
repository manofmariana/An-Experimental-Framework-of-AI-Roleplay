...
- **基于标签的内容激活系统**（与提示词组装系统是双生系统，一体设计）：顶层 TAG 过滤——所有注入组装前先按 tag 过滤，跨地点私密性（酒馆闲聊 + 电话悄悄话同组分流）由此实现，判定基于 channel 变量与角色持有标签。
- **三种输入模式**（本质是注入位置不同，属提示词组装的特殊子部分）：主控视角 / 上帝视角（"命运"段落注入 NPC，该轮 NPC 不注入 GM，NPC 活动只为后续轮次提供连续心理与目标）/ 写作指令（仅注入 GM，控制文风详略）。主控与上帝每轮二选一，写作指令可与任一共存。
- **变量/数值规则与前端状态栏模板**（人工设计一个版本，作为 TAG 系统与提示词组装系统的验证用例）：变量系统按存档/世界观自定义。
