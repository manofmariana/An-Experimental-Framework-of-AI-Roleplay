/**
 * HTTP API 路由表（优化阶段 D3，docs/optimization-review.md §9「服务端职责边界」）。
 *
...
export interface ApiDeps {
  ...
  /** config.json 路径（测试注入临时文件，不触碰真实配置） */
  configFile: string;
}
 ...
 * 写操作生效规则：config 域热重载立即生效；world/character 域 markStale 后下次 new_session 生效；
 * prompts 域热加载下一轮生效。
 */
