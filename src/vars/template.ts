 ...
 *
 * 变量模板（vars-template.json）= {world, character, types?} 三棵声明树：容器/末端
 * 两类节点，末端是唯一可携带 TAG、可被路径调用的单元。节点按字段判别：字符串简写 =
 * valueType 末端；{valueType, formula?} = 末端完整形；{children} = 容器；{type} =
 * 类型容器（子键 = 该类型的实例，实例名自由）。容器子键禁用 value/tags/formula
 * （消除实例简写二义性）。类型引用必须可解析且无环（成环 = 加载拒绝），解析期把
 * 被引用类型声明内联进类型容器节点，此后路径解析无需类型注册表。
 *
 * 末端可带 formula：{expr, binds?} = 数值公式（compileFormula 编译 + 变量闭包校验，
 * binds 值 = 同根模板路径、须解析到 number 末端）；{op: "union_attach", paths} =
 * 内置算子（paths = 同根模板路径、须解析到容器声明；空数组 = 仅自身 attachtags）。expr 只允许挂在 number 末端，
 * union_attach 只允许挂在 string_list 末端。
 *
 ...
 * {tags?, type}，TagEntry = {name, level} | {category, level}（恰居其一，level ∈
 * 1-7 整数）。parseVarsTags 做同构校验（每条路径须在模板中存在且容器/末端位置
 * 对应）；resolveAttachTags 把条目解析为 末端路径 → {name, level}[]：节点级条目
 * 向下级联到全部后代末端，末端级条目只挂本末端。{category} 条目：category 须在
 * 调用方注入的开放类别集合内；cid 类按属主分发（提供 ownerCid → {name: ownerCid}，
 * 未提供 = 报错，world 根解析不传 ownerCid），其余类别归一化为类别名记号。类型
 * 节点（{tags, type} 形式）的条目级联到该类型全部末端，路径中该层以 "*" 占位。
 */
import { z } from "zod";
...
/** 类型容器：子键 = 该类型的实例；decl = 解析期内联的被引用类型声明（无环保证有限）。 */
export interface TypeContainerDecl {
  kind: "typeContainer";
  typeName: string;
  decl: ContainerDecl;
}

export type DeclNode = TerminalDecl | ContainerDecl | TypeContainerDecl;
...
// ---------------------------------------------------------------------------
// 路径解析（声明树上；类型容器经内联 decl 穿越，实例名段自由）
// ---------------------------------------------------------------------------

/** 解析根内点分路径到声明节点；不存在/穿越末端 = 抛错（消息带路径）。 */
export function resolveDeclPath(root: DeclNode, dottedPath: string): DeclNode {
  ...
  for (const seg of dottedPath.split(".")) {
    ...
    if (node.kind === "container") {
      ...
      if (seg === "") {
        throw new Error(`模板路径 "${dottedPath}" 含空实例名段`);
      }
      node = node.decl;
    }
  }
 ...
 * + binds 路径可解析且指向 number 末端；union_attach = paths 可解析且指向容器声明。
 * rootDecl = 同根声明树（binds/paths 的解析基准）；atPath 仅用于报错定位。
 */
  ...
  if ("op" in raw) {
    ...
    for (const p of raw.paths) {
      ...
      if (target.kind === "terminal") {
        throw new Error(`union_attach 子树路径 "${p}" 必须解析到容器声明（${atPath}）`);
      }
    }
...
/** 类型引用图成环检测（DFS，消息带环路径）；引用未声明类型 = 抛错。 */
...
function convertNode(raw: RawNode, ctx: ConvertCtx, path: string, allowTagsChild = false): DeclNode {
  ...
  if ("type" in raw) {
    return { kind: "typeContainer", typeName: raw.type, decl: ctx.resolveType(raw.type) };
  }
  if ("children" in raw) {
...
/**
 * 解析并校验变量模板：zod 形状校验 + 语义校验（类型引用可解析且无环、容器子键
 * 保留名、公式闭包/路径校验、character 根保留名规则）。任何违规 = 抛错拒装。
 */
export function parseVarsTemplate(raw: unknown): VarsTemplate {
  ...
  const resolveType = (name: string): ContainerDecl => {
    ...
    if (decl.kind !== "container") {
      throw new Error(`类型 "${name}" 必须是容器节点`);
    }
...
/** 附加文件节点：{tags?, children?} 或 {tags?, type}。 */
...
const VarsTagsNodeSchema: z.ZodType<VarsTagsNode> = z.lazy(() =>
  z.union([
    z.object({ tags: z.array(AttachTagEntrySchema).optional(), type: z.string().min(1) }).strict(),
    z
      .object({
...
/** 同构校验：附加文件每条路径必须在模板中存在且容器/末端位置对应。 */
function checkIsomorphic(node: VarsTagsNode, decl: DeclNode, path: string): void {
  if (decl.kind === "terminal") {
    if (node.children !== undefined || node.type !== undefined) {
      throw new Error(`TAG 附加文件路径 "${path}" 在模板中为末端，不得带 children/type`);
    }
    return;
  }
  if (decl.kind === "typeContainer") {
    if (node.type !== undefined) {
      if (node.type !== decl.typeName) {
        throw new Error(`TAG 附加文件路径 "${path}" 的类型 "${node.type}" 与模板类型 "${decl.typeName}" 不符`);
      }
      return;
    }
    // children 形式：子键 = 实例名（自由），逐键对拍类型声明
    for (const [key, child] of Object.entries(node.children ?? {})) {
      checkIsomorphic(child, decl.decl, path === "" ? key : `${path}.${key}`);
    }
    return;
  }
  if (node.type !== undefined) {
    throw new Error(`TAG 附加文件路径 "${path}" 在模板中为普通容器，不得带 type`);
  }
  for (const [key, child] of Object.entries(node.children ?? {})) {
...
/**
 * 把附加条目解析为 末端路径 → {name, level}[]：节点级条目向下级联到全部后代末端，
 * 末端级条目只挂本末端；同名去重（先取者胜）。类型节点（{tags, type} 形式）的条目
 * 级联到该类型全部末端，路径中该层以 "*" 占位（运行期按实例名匹配）。
 */
export function resolveAttachTags(
  ...
  const cascadeDecl = (decl: DeclNode, path: string, inherited: readonly TagMount[]): void => {
    ...
    if (decl.kind === "typeContainer") {
      // 类型层无实例名可枚举，以 "*" 占位
      cascadeDecl(decl.decl, `${path}.*`, inherited);
      return;
    }
  ...
  const walk = (node: VarsTagsNode, decl: DeclNode, path: string, inherited: readonly TagMount[]): void => {
    ...
    if (decl.kind === "typeContainer") {
      if (node.type !== undefined) {
        cascadeDecl(decl.decl, `${path}.*`, mounts);
        return;
      }
      for (const [key, child] of Object.entries(node.children ?? {})) {
        walk(child, decl.decl, path === "" ? key : `${path}.${key}`, mounts);
      }
      return;
    }
