 ...
 *
 * 服务世界页「TAG 附加」子区：按 vars-template 声明树投影编辑视图（无实例列——结构化
 * 数组节点以 {tags, array} 形式整型挂载，扇出到元素结构全部末端（[*] 占位），不展开
 * 元素），每个节点可挂附加条目（{name, level} | {category, level}，level 1-7 整数；
 * 名称下拉自同包 tags.json 注册表，自由输入不受限——与实例 tags 编辑同一口径）。
 * character 根投影并入系统声明分支（镜像常量取自 system-char-decl.js；类型解析回退
 * 系统类型 relation），系统节点同样可挂附加条目——与服务端 parseVarsTags 按并入后
 * 根校验的口径一致。
 *
 * 工作副本 = 调用方深拷贝的 {world, character} 两附加根；编辑原地作用于副本：
 * - setNodeTags(root, path, entries)：整条目表替换——路径先对拍模板（不存在即抛；
 *   数组节点不穿越，指向数组 = 整型挂载），沿途物化稀疏节点，空条目 + 空子树回剪
 *   （保持文件稀疏）；path = "" = 根节点自身条目（根挂条目 = 级联到该根全部末端；
 *   character 根可挂 {category:"cid"} 按属主分发）；
 * - 数组节点：array 值 = 元素类型名（内联元素结构为 "*"）；已存在 children 旧形态
 *   时拒绝整型覆盖（不丢数据）；
...
export function createVarsTagsModel({ template, varsTags }) {
  ...
  /** 根内点分路径 → 声明信息（数组不穿越——附加树按声明层呈现，数组 = 整型挂载点；不可解析 = null）。 */
  ...
  /** 附加根上按声明路径取已有节点（数组层 = {array} 整型节点本身；无 = null）。 */
  ...
  function prune(root, path) {
    ...
    for (let i = segs.length; i >= 1; i--) {
      ...
      // 整型节点（{tags, array} 形式）有条目即活；普通节点有条目或有子树即活
      const alive = node.array !== undefined ? hasTags : hasTags || hasChildren;
      if (alive) {
  ...
  /**
   * 声明子树 → 附加编辑视图节点（tagsTerminal / tagsContainer / tagsArray）。
   * 数组不展开：entries = 整型（{array} 形式）条目；存在 children 旧形态时
   * hasLegacyChildren 标记（整型挂载被拒，旧子树原样保留不丢数据）。
   */
  function buildNode(root, key, path, raw) {
    ...
    if (info.kind === "array") {
      const node = tagsNodeAt(root, path);
      const hasLegacyChildren = node !== null && node.array === undefined && isPlainObject(node.children);
      return {
        key, path, kind: "tagsArray", elementType: info.elementType ?? null,
        entries: hasLegacyChildren ? [] : entriesAt(root, path),
        hasLegacyChildren,
      };
    }
    return {
  ...
  return {
     ...
     * 数组节点 = 整型挂载（{tags, array} 形式，array = 元素类型名/内联为 "*"）；
     * 已存在 children 旧形态即拒（不丢数据）。
     */
    setNodeTags(root, path, entries) {
      ...
      if (info.kind === "array") {
        ...
        if (existing !== null && existing.array === undefined && isPlainObject(existing.children)) {
          throw new Error(`路径 "${path}" 存在 children 旧形态附加（本编辑器不管理），拒绝整型覆盖`);
        }
      }
      if (list.length === 0) {
        const node = tagsNodeAt(root, path);
        if (node !== null) {
          delete node.tags;
          if (node.array !== undefined && info.kind !== "array") delete node.array;
        }
        prune(root, path);
        return;
      }
      ...
      if (info.kind === "array") {
        delete node.children; // 整型形态与 children 互斥（上面已拒旧形态，此为防御）
        node.array = info.elementType ?? "*";
      }
      node.tags = list;
    },
