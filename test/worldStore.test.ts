...
describe("WorldStore 新结构", () => {
  it("落盘仅 schema_version/world/pipeline，无 vars/clock", () => {
    const dir = tempDir("airp-world-"); const store = new WorldStore("t", { world: { time: start, weather: "fog" } }, dir);
  ...
  it("setClock 写 world.time 变更并可逆", () => {
    const dir = tempDir("airp-world-"); const store = new WorldStore("t", { world: { time: start } }, dir);
