...
function state(overrides?: Partial<CharacterProjectionInput>): CharacterProjectionInput {
  return {
    ...
    initiative: null,
    relations: { C1002: { name: "周砚", impression: "可信" } },
...
describe("系统声明分支并入", () => {
  it("character 根并入系统子树；characterVars 只含世界作者声明；types 并入 relation", () => {
    ...
    // 系统路径可解析（投影视角）：location 容器 / relations 类型容器穿越
    assert.equal(TPL.resolve("character", "location.name").kind, "terminal");
    assert.equal(TPL.resolve("character", "relations.C1002.impression").kind, "terminal");
  });
  ...
  it("character 根挂条目级联到系统分支全部末端（cid 类别按属主分发）", () => {
...
describe("projectCharacterTree", () => {
  it("系统分支值从类型化字段读出为标准末端外壳；vars 树原样并入", () => {
  ...
  it("系统末端外壳 tags 取自 systemTags 侧车", () => {
    const tree = projectCharacterTree(
      state({ systemTags: { name: [{ name: "闻名", level: 3 }], "relations.C1002.name": [{ name: "旧识", level: 1 }] } }),
    ) as Record<string, unknown>;
    assert.deepEqual(tree["name"], { value: "林雾", tags: [{ name: "闻名", level: 3 }] });
    const rel = tree["relations"] as Record<string, unknown>;
    assert.deepEqual(rel["C1002"], {
...
describe("validateSystemTags", () => {
  it("合法侧车原样返回（含 relations 实例路径）", () => {
    const sidecar = { name: [{ name: "闻名", level: 3 }], "relations.C1002.impression": [{ name: "旧识", level: 1 }] };
    assert.deepEqual(validateSystemTags(sidecar, TPL.character), sidecar);
  });

  it("系统分支外路径 / 不可解析 / 解析到容器 = 拒绝", () => {
