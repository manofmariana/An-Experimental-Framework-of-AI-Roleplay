...
describe("CharactersStore 同构角色文件", () => {
  it("无 gm key、无 persona/voice，timer=start绝对分钟+偏移", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "airp-chars-")); const store = CharactersStore.initFrom("t", [manifest()], 1000, dir);
  ...
  it("旧 schema 拒绝并给新建会话提示", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "airp-chars-")); fs.writeFileSync(path.join(dir, "characters.json"), JSON.stringify({ C1001: {} }));
