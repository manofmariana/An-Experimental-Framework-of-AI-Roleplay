...
describe("prose 输入", () => {
  ...
  it("模板不含 GM 公共长期记忆与 voice anchors", () => {
    const template = loadTemplate("prose", Object.keys(PROSE_PLACEHOLDERS), FACTORY_PROMPTS_DIR); const text = compilePrompt(template, PROSE_PLACEHOLDERS, context).map((message) => message.content).join("\n");
