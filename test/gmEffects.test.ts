...
function makeTruth(): TruthStores {
  ...
  truth.sys.setPipeline({ working_set: [{ cid: "C0" }, { cid: "C1001" }] });
  return truth;
}
...
describe("planGmAdjudication（提交形状）", () => {
  it("deltas/timer/location/周期与触发复位/acted 清零/组派生/工作集清算", () => {
    ...
    // 周期计数 X 清零 + 触发复位（sys 根程序计数键）
    assert.equal(truth.sys.counters.cycles_since_gm, 0);
    assert.equal(truth.sys.counters.gm_trigger, false);
    // 结算成员 acted 清零
    ...
    for (const p of ["world.region.fog", "characters.C0.timer", "characters.C1001.timer", "sys.cycles_since_gm", "sys.gm_trigger", "characters.C0.acted", "characters.C1001.group"]) {
  ...
  it("events 提交形状：ID 经注入分配器、tags 缺省 = 本轮行动者、location 透传、seq/t 落账", () => {
    ...
    assert.deepEqual(committed[0], {
      id: { value: "evt_0001", tags: [] },
      t: { value: clock, tags: [] },
      seq: { value: 9, tags: [] },
      kind: { value: "world", tags: [] },
      content: {
        value: "事件一",
        tags: [
          { name: "C0", level: 1 },
    ...
    assert.deepEqual(committed[1], {
      id: { value: "evt_0002", tags: [] },
      t: { value: clock, tags: [] },
      seq: { value: 9, tags: [] },
      kind: { value: "world", tags: [] },
      location: { value: "灯塔", tags: [] },
      content: { value: "事件二", tags: [{ name: "C0", level: 1 }] },
    });
...
describe("planGmAdjudication（重放等价：正常裁决与编辑重放同一规划器）", () => {
  ...
  it("编辑重放 = 反转旧 effects + 事件截断 + 同一 planner：终态与全新规划逐字节一致", () => {
    ...
    replayed.sys.setPipeline({ working_set: [{ cid: "C0" }, { cid: "C1001" }] });
...
describe("GM deltas 写入通道（applyVarDeltas：双根路由 + 模板校验）", () => {
  ...
  it("拒写 world.time / sys 程序分支 / 无声明路径（抛错拒绝该条；之前的 delta 已落账，原子性归 draft 丢弃）", () => {
    ...
    // sys 第五根不开放 GM deltas：程序分支键不在 world 变量树内，模板解析即拒
    assert.throws(planWith([{ path: "world._sys.cycles_since_gm", op: "=", value: 9 }]).plan, /不可解析/);
