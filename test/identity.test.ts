...
describe("renderForReader（身份替换）", () => {
  ...
  it("relations 有 name → 真名", () => {
    assert.equal(
      renderForReader("@C0推门进来", "C1001", [{ cid: "C0", name: "阿青" }]),
      "阿青推门进来",
    );
  });
  ...
  it("只有 impression → 印象称谓", () => {
    assert.equal(
      renderForReader("@C0推门进来", "C1001", [{ cid: "C0", impression: "鞋上有青苔的男人" }]),
      "鞋上有青苔的男人推门进来",
    );
  });
  ...
  it("name 优先于 impression；空串按缺省处理", () => {
    assert.equal(
      renderForReader("@C0坐下", "C1001", [{ cid: "C0", name: "阿青", impression: "旅人" }]),
      "阿青坐下",
    );
    assert.equal(renderForReader("@C0坐下", "C1001", [{ cid: "C0", name: "" }]), "陌生人坐下");
  });
...
describe("renderRefs*（正文指称占位符 [[称呼|@CID]]）", () => {
  ...
  it("reader 视图：relations 有 name → 真名", () => {
    assert.equal(
      renderRefsForReader("[[旅人|@C0]]推门进来", [{ cid: "C0", name: "阿青" }]),
      "阿青推门进来",
    );
  });
  ...
  it("reader 视图：只有 impression → 印象称谓兜底", () => {
    assert.equal(
      renderRefsForReader("[[旅人|@C0]]推门进来", [{ cid: "C0", impression: "鞋上有青苔的男人" }]),
      "鞋上有青苔的男人推门进来",
    );
    assert.equal(
      renderRefsForReader("[[旅人|@C0]]坐下", [{ cid: "C0", name: "", impression: "旅人" }]),
      "旅人坐下",
    );
  });
