...
describe("结构时间与角色变更回溯", () => {
  it("world.time 跨年推进后通过 VarChange 逐字节恢复", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "airp-rb-")); const world = new WorldStore("t", { world: { time: start, hp: 10 } }, dir);
  ...
  it("characters relations/timer/long_term_memory 倒序恢复", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "airp-rb-")); const store = CharactersStore.initFrom("t", [manifest], 100, dir); const initial = JSON.stringify(store.all());
