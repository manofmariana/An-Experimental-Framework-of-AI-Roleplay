 ...
 * - 视图模型：declTerminal/declContainer/declTypeContainer 投影、formula 结构化视图、
 *   character 根必需声明 canDelete 保护、类型区 typeRoot/声明字段投影；
 * - 结构新增：扁平末端（字符串简写声明）/ 结构体（{children:{}} 可递归）/ 多实例容器
 *   （{type} 引用已有类型）；保留名/冲突/程序键拒绝；character 根必需声明补齐豁免；
 * - 删除声明：摘声明（容器级联）、character 根必需声明保护、类型声明内拒绝；
...
import {
  ...
  type DeclTypeContainerNode,
  type TypeDeclTerminalNode,
  type TypeDeclTypeRefNode,
  type TypeRootNode,
  type VarDeclModel,
  type VarDeclNode,
...
// ---------------------------------------------------------------------------
// 夹具：原始模板（字符串简写 / 完整形末端 / 容器 / 类型容器 / 类型区）
// ---------------------------------------------------------------------------
...
describe("var-decl-model：视图模型", () => {
  it("声明树投影：末端 valueType/formula 视图、容器递归、类型容器不展开", () => {
    ...
    const bag = childAt(view, "bag") as DeclTypeContainerNode;
    assert.equal(bag.kind, "declTypeContainer");
  ...
  it("buildTypesView：类型根 + 声明字段（末端/容器/{type} 引用）；listTypeNames", () => {
    const m = modelOf(makeTemplate());
    ...
    const parts = childAt(item, "parts") as TypeDeclTypeRefNode;
    assert.equal(parts.kind, "typeDeclTypeRef");
    assert.equal(parts.refTypeName, "part");
  });
...
describe("var-decl-model：结构新增", () => {
  ...
  it("多实例容器：{type} 声明；未声明类型拒绝", () => {
    const t = makeTemplate();
    const m = modelOf(t);
    m.addDecl("world", "", { name: "warehouse", kind: "typeContainer", typeName: "item" });
    assert.deepEqual((t.world.children as any).warehouse, { type: "item" });
    assert.throws(
      () => m.addDecl("world", "", { name: "bad", kind: "typeContainer", typeName: "nosuch" }),
      /未声明/,
    );
  });

  it("保留名/分隔符/冲突/程序键/未知种类一律拒绝；类型声明内不开放", () => {
    const m = modelOf(makeTemplate());
    ...
    assert.throws(() => m.addDecl("world", "bag.铁剑", { name: "x", valueType: "number" }), /类型声明不开放/);
...
describe("var-decl-model：类型区", () => {
  it("新建类型 = 命名 + 空结构体；随后逐字段定义（含结构体嵌套与 {type} 引用）", () => {
    const t = makeTemplate();
    const m = modelOf(t);
    ...
    m.addTypeField("tool", "", { name: "parts", kind: "typeContainer", typeName: "part" });
    assert.deepEqual((t.types as any).tool.children, {
      name: "string",
      count: "number",
      attachtags: "string_list",
    ...
    assert.throws(() => m.addTypeField("tool", "parts", { name: "x", valueType: "number" }), /请到类型/); // 穿越 {type} 引用
  ...
  it("删除类型：未引用可删；被 {type} 容器引用前端预检拒绝", () => {
    const t = makeTemplate();
    const m = modelOf(t);
    m.addType("tmp");
    m.deleteType("tmp");
    ...
    assert.throws(() => m.deleteType("item"), /引用/); // world.bag / character.gear 引用
    assert.throws(() => m.deleteType("part"), /引用/); // item 声明内引用
...
describe("var-decl-model：formula 声明编辑", () => {
  ...
  it("union_attach：仅 string_list 末端；paths 须解析到同根容器声明", () => {
    const t = makeTemplate();
    const m = modelOf(t);
    ...
    assert.throws(() => m.setDeclFormula("world", "pool", { op: "union_attach", paths: ["loc.name"] }), /容器声明/);
    assert.throws(() => m.setDeclFormula("world", "pool", { op: "union_attach", paths: ["nosuch"] }), /不可解析/);
  });

  it("expr：仅 number 末端；binds 须解析到同根 number 末端；标识符合法", () => {
    const m = modelOf(makeTemplate());
...
describe("var-decl-model：类型内 formula 编辑", () => {
  it("expr 挂上（简写升级完整形）→ 视图从动；清空变回普通末端；binds 以类型根为基准", () => {
    const t = makeTemplate();
    const m = modelOf(t);
    m.setTypeDeclFormula("item", "count", { expr: "w + 1", binds: { w: "parts.刀刃.w" } });
    assert.deepEqual((t.types as any).item.children.count, {
      valueType: "number",
      formula: { expr: "w + 1", binds: { w: "parts.刀刃.w" } },
    });
    const view = m.buildTypesView();
    const item = view.children[0] as TypeRootNode;
    const count = childAt(item, "count") as TypeDeclTerminalNode;
    assert.equal(count.derived, true);
    assert.deepEqual(count.formula, { kind: "expr", expr: "w + 1", binds: { w: "parts.刀刃.w" } });

    m.setTypeDeclFormula("item", "count", null);
    assert.deepEqual((t.types as any).item.children.count, { valueType: "number" });
  });

  it("binds 可穿越嵌套 {type} 引用解析（与服务端内联口径一致）；跨类型末端拒绝", () => {
    const t = makeTemplate();
    const m = modelOf(t);
    // parts = {type: part} 嵌套引用：part.w 可解析
    m.setTypeDeclFormula("item", "count", { expr: "w * 2", binds: { w: "parts.刀刃.w" } });
  ...
  it("union_attach 仅 string_list 末端；paths 以类型根为基准解析到容器", () => {
    const t = makeTemplate();
    const m = modelOf(t);
    ...
    assert.throws(() => m.setTypeDeclFormula("item", "attachtags", { op: "union_attach", paths: ["count"] }), /容器声明/);
  });

  it("嵌套 {type} 引用内的末端不开放（到其类型上编辑）；不可解析拒绝", () => {
    const m = modelOf(makeTemplate());
    assert.throws(() => m.setTypeDeclFormula("item", "parts.刀刃.w", { expr: "1" }), /请到类型/);
...
describe("var-decl-model：系统声明分支并入", () => {
  it("character 根视图 = 系统分支（system 标记 + 全操作禁）+ 作者子树原序；world 根不变", () => {
    ...
    // relations = 系统类型容器（declTypeContainer，typeName relation）
    const relations = childAt(view, "relations") as DeclTypeContainerNode;
    assert.equal(relations.kind, "declTypeContainer");
  ...
  it("系统路径一切写操作抛错，工作副本不被波及", () => {
    ...
    assert.throws(() => m.deleteDecl("character", "relations.艾.name"), /系统声明分支/);
  ...
  it("作者 formula 可绑系统 number 末端 / union_attach 可指系统容器（与服务端并入后解析口径一致）", () => {
    const t = makeTemplate();
    const m = modelOf(t);
    ...
    m.setDeclFormula("character", "tags", { op: "union_attach", paths: ["gear", "location"] });
    assert.deepEqual((t.character.children as any).tags.formula, { op: "union_attach", paths: ["gear", "location"] });
